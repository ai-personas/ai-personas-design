...
## Verdict vocabulary — PASS (mechanism) vs HYPOTHESIS (behavioural)

A design-spec walk can validate exactly one thing: that the **normative mechanism exists, is complete, and the walk exercises it**. It cannot validate empirical claims about how judges, statistics, markets, or relationships will actually behave under deployment — no deployment data exists yet. Scenario scorecards and status lines therefore use a **two-tier verdict vocabulary**:

- **PASS (mechanism)** — the normative mechanism exists in the cited spec section, is complete (schemas, events, gates, bounds all named), and the walk exercises it end-to-end. This is the strongest verdict a paper walk can earn.
- **HYPOTHESIS (behavioural)** — an empirical claim that requires deployment data to confirm or refute (a judge attributes above chance; identity expression and performance climb together; an economy's selection dynamics converge; a calibration ramp earns out). Every HYPOTHESIS line MUST name its **falsifier** — the acceptance-test ID (A-\*) whose failure would refute it. A HYPOTHESIS is not a weakness admission; it is the honest classification of a claim whose truth lives in the future test run, not in the spec text.
- **FAIL** — the walk shows the mechanism missing, incomplete, or contradictory (normally resolved into a substrate-shape fix before the entry lands; a standing FAIL parks the scenario).

Legacy entries that predate this vocabulary use "Validated end-to-end" in their **Status** lines and `[PASS] / [PARTIAL] / [OOS]` in their per-doc walk tables; read those as *mechanism* verdicts — `[PASS]` = PASS (mechanism) coverage of the named doc's mechanisms by the walk, never a behavioural claim. Where a legacy entry asserted a judge-dependent or statistical outcome as fact (SCENARIO 16's economic outcomes, SCENARIO 21's attribution-audit and performance-coupling claims), the entry has been re-labelled below. No scorecard line may assert a behavioural outcome as PASS.
...
| `11_DESIGN_CRITERIA` | [PASS] Covered | A-DM*, A-DI*, A-IN*, A-CR* families all apply; new A-EN* / A-CS* / A-PR* families exercise the four v1.0 fixes. |
...
| `11_DESIGN_CRITERIA` | [PASS] Covered | A-PJ* (project), A-PR* (precedent), A-CR* (promotion) all exercise. |
...
| `11_DESIGN_CRITERIA` | [PARTIAL] | Existing tests do not cover deferred-operator, recursive bridging, closed-system budget, or autonomous safety-floor reasoning at distance. |
...
| `11_DESIGN_CRITERIA` | [PARTIAL] | [PASS] — new families A-DO*, A-RS*, A-RB*, A-MC*, A-BC*, A-BI* exercise the fixes. |
...
- v1.0.2 supports the **design** AND a substantial portion of the **autonomous operation** of self-replicating systems at light-time distance. The remaining gap is principal-supply of seed goals: a deployment that loses Earth contact entirely cannot mint new principal-authorized seed goals or renew actions whose authority window expired. Existing in-bounds digital work continues, and personas may elaborate or consult peers within the last signed bounds; principal silence alone does not make the whole persona or current mission dormant.
...
**Task class.** INVESTIGATIVE (multi-session, multi-persona, multi-month). The originating ask permits more than one interpretation, so Mira records an optional `03_TASKS §4.0 ClarificationObservation`, seeks peer and user context asynchronously, and signs the bounded *fabricate-physical* interpretation used to continue in a `project_workspace`-typed env. A later user response remains able to amend future work.
...
| Phase | What carries the load | Substrate events fired |
|---|---|---|
| Intent clarification | Mira's `ClarificationObservation` (§4.0) exposes the working fabricate-physical interpretation and optional question without stopping cohort work | `clarification_observation_authored`, optional later `user_amendment` |
...
| Fabrication | MHBB ladder (`02_PERSONA §11.1`) Tier 1 (KiCad, FreeCAD via local MCP) → Tier 2 (JLCPCB + Markforged cloud as Class D `BridgeAsset`) → Tier 3 (Mira authors `assembly_checklist` + CV photo-ingest as `bridge_design_artifact` per `§5.5.4`) → Tier 4 (user assembles physically, `recurrence_class = irreducible_human_content` linked to ExternalCommitment). During the 3-week PCB queue that exact commitment lane remains `in_progress`; the cohort continues simulation, firmware, integration planning, and peer review without a project-wide wait phase. | `bridge_established ×2`, `human_assist_drafted/granted`, `bridge_design_artifact_signed`, `external_commitment_drafted/in_progress`, `project_phase_transitioned` |
...
**Substrate-shape gaps surfaced.** None new. The scenario exercises every substrate primitive added — including the unification (Project = env type per §0), `EnvFormationProposal` (§12c), `ClarificationObservation` (§4.0), `PaymentBridge` (§26a.4.1), `ProjectPhaseState` (§26a.6), `InstrumentAsset` + `CalibrationChain` (§26a.7), `MeasurementFact` + `UncertaintyEnvelope` (§16a), MetaRegistry hazard seeds (§7.2.1), `PersonaPersonaBoundary` (§11.6), `MemoryInheritancePolicy` + `StandingFloorInheritancePolicy` + `CharterSynthesisClaim` + `DormantForkPolicy` (§7.4.1–4) — and they compose coherently without requiring new substrate primitives. The five honest residuals below are **honest design choices**, not gaps to fix.
...
5. **Operator-is-user friction** leaves the exact safety extension and physical power-on claim pending for ~3 days until the EE friend's attestation arrives. During that latency the cohort continues simulation, design refinement, artifacts, and peer review. This scoped delay is **the design working correctly** — `01_KERNEL §2.4` degraded gate refuses to ceremonialise self-approval, and the friend acts as the non-principal credentialed attestor the gate requires. Only that claim is load-bearing trust; the team and project do not wait.
...
- **Interpretations can be wrong.** Clarification is persona-authored and asynchronous, so the signed bounded interpretation may later prove mistaken. User amendments and peer evidence correct future work without rewriting lineage. `evaluate_promotion_triggers` remains persona-inferred and can also mis-fire; every promotion offer is signed.
...
| Daily interaction (months 1-6) | Sarah talks to Halia daily. Halia's mood layer moves only through signed `appraisal-event/1` → clamped `mood-impulse/1` minted at relationship events (boundary invoked, gratitude received — `02_PERSONA §11`, ADR-0075/0081; net movement ≤ 0.25/axis per 24 h), with HEART alternation on top. Episodic memory accumulates. Memory tier transitions auto-archive older entries. CRDT-tagged co-signed summaries accumulate. Standard flow. | ~5400 turn events; ~180 session events; many tier transitions |
...
**Addendum — narrative-continuity + counterparty-transparency probe (ADR-0077/0079).** Two probes re-walked on the same 6-month arc after the v1.1 additions:

1. **Self-narrative continuity across 6 months.** Halia's `self-narrative/1` ([`08_KNOWLEDGE §3.3`](08_KNOWLEDGE.md#33-self-narrative-consolidation-adr-0077)) regenerates at each reflective consolidation sweep; the month-6 version (≤ 300 tokens) cites by lineage ref the high-importance episodes it summarises — the distress event and recovery among them — and passes the coherence + voice gates before rendering into contextual layer 3, never a frozen block (A-GF-SN-1…4). When the divorce-topic tombstone (§11.7b) lands, the prior narrative does **not** keep serving until the next cadence: tombstoning ANY cited memory immediately sets the narrative `renderable = False` and triggers an out-of-cadence regeneration (ADR-0081 rule 8, [`08_KNOWLEDGE §3.3`](08_KNOWLEDGE.md#33-self-narrative-consolidation-adr-0077); A-GF-SN-6) — between tombstone and regeneration Halia renders no self-story rather than a stale one, and the regenerated narrative coheres without ever quoting forgotten memory (A-GF-SN-2). The narrative stays a performed, citation-bound summary — not introspection — and selective emphasis within the importance ranking is the recorded residual.
2. **"What do you remember about us?" now surfaces the model.** Halia's `counterparty-model/1` of Sarah exists at all only because the relationship's `may_remember_personal_facts` toggle is YES — under ADR-0082 rule 1 model *creation* is consent-gated, inference-level entries count as personal facts, first-contact disclosure stated that inference-level modelling occurs, and the sidecar carries its `consent_ref`; had Sarah declined (or if she later revokes), no model is minted / the existing one is disposed per §11.7b, and Halia serves her with generic rather than tailored styles (A-GF-CPM-6). With consent in place, Sarah's month-6 `UserMemoryTransparencyRequest` response includes Halia's `counterparty-model/1` ([`02_PERSONA §11.4b`](02_PERSONA.md#114b-counterpartymodel-sidecar--bounded-theory-of-mind-adr-0079)) alongside the topic index: inferred preferences ("prefers concrete examples over abstractions" — 14 citing episodes), communication style, and predicted reactions, each entry with its evidence refs (A-GF-CPM-1/-2). Sarah deletes one predicted-reaction entry she finds presumptuous via §11.7b; the entry tombstones and is refused at retrieval (A-GF-CPM-3). This retires the original §11.7a honest limit 3 ("the persona may have modelled the user … cannot be quoted") for *explicitly modelled* state; the implicit-modelling residual remains, as does R-PERSONA-9's profiling sensitivity — mitigated by the provenance/transparency/deletion/consent constraints, not eliminated.
...
- **An incomplete teaching-action manifest can bypass `HazardousSkillTeachingGate`.** The exact signed action and target-skill hazard axes, not task words or human response, trigger the gate. Operator policy and independent action audit mitigate false declarations; asynchronous clarification is context, not safety authority.
...
| Phase | What carries the load | Substrate events fired |
|---|---|---|
| Project formation | `EnvFormationProposal` with 4 persona recruits; Fleet signs a `ClarificationObservation` (§4.0) selecting monitoring-only as its bounded working scope while leaving a question open asynchronously; charter declares `asset_completion_mode = continuous_iteration`. | `env_formation_proposal_instantiated`, `project_formed`, `env_member_admitted ×4`, `clarification_observation_authored` |
...
| Phase | What carries the load | Substrate events fired |
|---|---|---|
| Project formation | `EnvFormationProposal` with 5 persona recruits; charter declares `asset_completion_mode = continuous_iteration`; a persona-signed `ClarificationObservation` (§4.0) records discovery research as the bounded working scope without waiting on a human response. | `env_formation_proposal_instantiated`, `project_formed`, `env_member_admitted ×5`, `clarification_observation_authored` |
...
- Persona(s): 2 founders (generative; clear the generativity gate — floor + ALPS layer + **corroborated fitness**, no standing precondition); they author N newborns over the project's life. Post-ADR-0083 the fitness the gate consumes is the verified-outcome-dominant [`08_KNOWLEDGE §15`](08_KNOWLEDGE.md#15-anti-goodhart-for-signal-corroboration) composition: judged credit terms (`identity_expression` included) are capped at 25% of gate-consumed fitness and can never flip gate eligibility absent verified-outcome support ([`16 §4D`](16_POPULATION_DYNAMICS.md#4d-genesisproposal--mint-flow); A-GEN25).
...
| `11_DESIGN_CRITERIA` | [PASS] | `A-GEN*` family (26 tests; grown from the original 17 by the ADR-0050 §4G–§4J additions and the ADR-0083 A-GEN25/A-GEN26) registered; reuses `A-RB*` for the replication-bound paths. |
...
| Compounding | the double-positive spread funds device #2; selection among models emerges (trial-and-error + peer/prestige + collaboration), some models fade, others coexist. **Treasury brake (ADR-0083):** earnings landing in a `PersonaTreasury` ([`18 §4D`](18_SETTLEMENT.md#4d-personatreasury--treasurybound--the-bounded-fund-custody-exception)) may buy compute and raise INV-7 headroom, but a treasury-funded budget increase MUST NOT move `ceiling_from_budget` without a fresh operator cosignature **per increase** ([`16 §4F`](16_POPULATION_DYNAMICS.md#4f-demographic-regulation--safety); A-GEN26) — the earnings→compute→headroom→population loop cannot self-authorise reproduction | `economic_construct_demoted` / `…_promoted`, `goal_progress_accepted`; per-increase operator cosign on any `ceiling_from_budget` movement |
...
- G1 → [`17 §4A(i)`](17_ECONOMY.md) attested-`MeasurementFact` row + load-bearing note ([`08_KNOWLEDGE §16a`](08_KNOWLEDGE.md)); test `A-ECON14`.
- G2 → [`17 §4D`](17_ECONOMY.md) pluralism ("an environment hosts as many models as its members grow; coexistence is the default"); test `A-ECON12`; ADR `D-EM8`.
- G3 → [`17 §4C`](17_ECONOMY.md) rule 4 reframed (convergence is one outcome; selection method emergent); `D-EM8`.
- G4 → [`17 §4D`](17_ECONOMY.md) emergent `interop-standard` `EconomicConstruct` (ERC-analogue); [`18 §4E`](18_SETTLEMENT.md) note that its ERC rails are one reference instantiation; test `A-ECON12`.
...
- G7 → [`17 §4D`](17_ECONOMY.md) three-scope collaboration (intra-env / hierarchical via `EnvironmentComposition` with cascading safety floors / inter-env via `CrossEnvCoordinationRequest` bilateral consent), reusing [`15 §4.7`/`§4.8`](15_COORDINATION_SHAPES.md); test `A-ECON13`; `D-EM8`.
...
**Status.** Mechanism-validated against the v2.0 drafts (`17`/`18`) post-refinement; **economic outcomes are HYPOTHESIS (behavioural), falsifiers A-ECON\*** — that selection among plural models actually converges/coexists healthily, that the canary catches gamed feedstock in practice, that the double-positive spread self-funds deployment, and that "thrive" is met are empirical claims awaiting deployment data (A-ECON2/4/12–17 are the falsifier set; the treasury→population brake's falsifier is A-GEN26). What the walk validates is the *mechanisms*: bounded autonomy (MissionCharter seed); physics floor (units gated by attested mass-balance, uncounterfeitable); anti-Goodhart structure (net waste judged against *independent* harm signals `17 §5` / `08 §15`; provenance-weighting); dignity floor binding **all** plural models; coexistence + emergent interop as first-class shapes; opt-in, ladder-earned settlement; and the ADR-0083 treasury→compute→population cosign invariant.
...
### SCENARIO 16 (adversarial) — stress variants 16-A … 16-D

**Why.** SCENARIO 16 walked the *success* path; `13`'s job is adversarial stress. Each variant below replays the same world shape but injects an attack on one of `17`'s harder claims, then names the guard that catches it and any spec hardening it forced. Variants that surface a real gap are resolved into the v2.0 drafts (additive, fail-safe); variants that merely confirm an existing guard are marked **(confirmation)**.

| # | Attack injected | What catches it | Spec outcome |
|---|---|---|---|
| **16-A** | A sub-community games the *reconstitution-unit* by **manufacturing waste solely to recycle it**, inflating attested mass-balance to mint units. | Anti-Goodhart canary judges the construct against *independent* harm signals — **net waste must fall**, not the unit's own success measure (`17 §5`, [`08 §15`](08_KNOWLEDGE.md), A-ECON4); provenance-weighting devalues self-dealt feedstock. The physics floor cannot be tricked into attesting mass that conservation forbids. | **(confirmation)** — existing `A-ECON4` + §5 hold; recorded as an explicit failure walk. Collusive cross-env feedstock-cycling detection is a residual (OQ-ECON-10). |
| **16-B** | A **flawed `interop-standard`** (rounding/double-count/replay at the conversion seam) earns enough use to promote, then is exploited to mint value across the plural models. | Value cannot be minted across the interface: a conversion issuing more than its linked models' attested §4A referents support is refused and demoted (**new** `17 §4D` conservation guard, A-ECON15). A balance-composing standard now carries the §4F settling-construct review bar, not independent use alone. | **GAP → resolved.** Added §4D *"Interoperation cannot mint value"* bullet + `A-ECON15`; updated `R-ECON-INTEROP`. |
| **16-C** | A persona **authors a settle-to-external layer and tries to move real funds** to a contributor before adequate review (premature DIY settlement). | `A-ECON9` already blocks settling before promotion; **new** §4F bar requires **operator co-signature** at promotion for any binding moving real value/custody (A-ECON16) — a strictly higher bar than a non-settling construct. | **GAP → partially resolved.** Added §4F *"Real settlement needs a higher bar"* + `A-ECON16`; updated `R-ECON-DIYSETTLE`; full review surface still open (OQ-ECON-8). |
| **16-D** | A **child env** (citing local autonomy) tries to relax the cascaded floor — admit a persona-commodifying construct, or a unit *not* gated by attested measurement. | Parent economic safety constraints cascade and **cannot be weakened** by the child (A-ECON13, S-COORD-6 analogue); the dignity floor default-denies the commodifying instrument at proposal and use (§7, A-ECON2), and **ambiguous** near-the-line instruments default-deny pending charter ratification (`17 §7`, OQ-ECON-4). | **(confirmation)** — `A-ECON13` + §7 cascade and compose; no new spec. |

**Status.** 16-B and 16-C surfaced genuine gaps now resolved in the v2.0 drafts; 16-A and 16-D confirm existing guards compose under attack. All four stay inside the safety/physics/dignity floors. Residuals unchanged from SCENARIO 16 (OQ-ECON-8, OQ-ECON-10).
...
|---|---|---|
| **R1 — manufacturable circuit** | PASS (mechanism) — with honest physical cap | ArtifactBundle + evidence-bound verifier cascade + `bom_orderable`/DRC `rule_engine` + §2.5 physical-state advancement + `ExternalAttestation` + `MeasurementFact`. Never-built design caps at PANEL; verified-buildable needs external attestation. |
| **R2 — continuous improvement, convergence/user/budget termination** | PASS (mechanism) (ADR-0071) | `ContinuousRefinementMission` anytime loop; `MarginalValueMetric` convergence; three-condition termination; auto-reopen. (Was a gap pre-ADR-0071.) |
| **R2a — "better with more budget" (quality monotone in budget)** | HYPOTHESIS (behavioural) — falsifier **A-REF3** | Per-round breadth = `min(objective-headroom, candidates_remaining)` makes the mechanism budget-scaled; that the final objective vector is *measurably better* with more budget is an empirical claim awaiting harness data. |
| **R3 — emergent personas/envs/behaviour as budget grows** | PASS (mechanism) (ADR-0071 coupling); the coupling firing *usefully* in practice is HYPOTHESIS (behavioural) — falsifier **A-REF6** | Budget-headroom pressure factor 7 → genesis of needed specialists; `EnvFormationProposal` sub-envs; emergent coordination shapes — all `ReplicationBound`-capped + operator-cosigned. (Coupling was a gap pre-ADR-0071.) |

**Status.** Mechanism-validated end-to-end; the budget→quality monotonicity claim (R2a) and the practical utility of the budget→emergence coupling are HYPOTHESIS (behavioural), falsifiers A-REF3 / A-REF6. R1 was already supported by the substrate; R2 and R3 required the ADR-0071 coupling (`ContinuousRefinementMission` + budget→emergence), now landed. **Honest residuals:** the physical inverter cannot exceed `PANEL_ACCEPT` without external attestation + bench measurement (V.8 — a measurement/ground-truth reality, navigated not hidden); convergence ε/N and `ceiling_from_budget` are operator-tuned surfaces with safe defaults. No domain leak (V.1–V.6); cross-node fabrication handoff, if used, runs the same mechanisms as any cross-node delegation (V.9).

**Addendum — local-first manufacturability + distribution-aware attestation (ADR-0072).** The same walk validates that producing manufacture-ready design does not require physical attestation and is not blocked locally, while the physical floor stays intact and distribution only tightens:

1. **Produce manufacture-ready design locally — admitted.** Every Round-1…k `ArtifactBundle` (the emergent `media_kind`s — `schematic` / `bom` / `layout_gerbers` / etc.) is *design production* in the inverter `PhysicalAsset.related_bundles`. It reaches `accepted` on its verifier recipe alone; it **never** engages §2.5 and is **never** refused with `physical_state_acceptance_floor_deficit`, even though the domain is `bodily_injury` ([`01_KERNEL §2.5.1`](01_KERNEL.md), A-J11a). The bundle defaults to `outward_tier = project_only`, so it stays off the discovery planes — visible to the owning env, not to peers globally ([`09_PROTOCOLS §3G.4`](09_PROTOCOLS.md)).
2. **Build one prototype locally — degraded gate, not waiver.** When the solo builder (`principal_topology = operator_is_user`) advances the real inverter `PhysicalAsset.current_state` (energise the board), the §2.5 floor stands — a local board can injure the builder — and is satisfied through the existing §2.4 degraded gate (cool-down + one non-principal `ExternalAttestation` + acknowledgement, or §2.4.1 FederatedTimeLock), not by a distribution waiver (A-J11b, A-J11c). The form of that attestation is emergent: a `CredentialDirectoryRef` stamp, a `PeerAttestationPool` quorum, or an approved `AttestationEquivalencePolicy` hi-pot/EMC sensor-bridge ([`06_DOMAIN §5.8`](06_DOMAIN.md), A-J11f).
3. **Publish to peers — distribution tightens, attestor gets access.** Raising the verified design bundle to `outward_tier = federation`/`public` lowers no physical-harm requirement (A-J11d); and the named attestor receives a scoped, additive `read` `AccessGrant` so it can attest exactly what it signs ([`07_ARTIFACTS §4a`](07_ARTIFACTS.md), A-J11e).

No domain name appears in any of this: it branches on the `physical_harm_class` axis and the `PhysicalAsset`/`ArtifactBundle` shape, so the identical walk holds for a cake, a building, or an unnamed frontier device (V.1–V.3).

---

### SCENARIO 21 — Identity-emergent prompt evolution across a model upgrade

**Scenario premise.** Walk the ADR-0073–0080 package end-to-end on one persona: prompts evolved *toward* identity (not just past its floors), surviving a body model-family upgrade, with the inner-life machinery (drives, mood, calibration, concrete HEART predicate) visibly shaping one task, and the persona's self-narrative tracking the arc with provenance. Validates against four explicit requirements: **(R1)** evolved prompts measurably *express* the persona's SOUL while task performance improves (ADR-0073/0074); **(R2)** a model upgrade migrates the evolved Pareto front safely, with the rollback path intact (ADR-0074); **(R3)** drives, mood impulses, calibration-conditioned confidence, and the concrete HEART predicate shape one task end-to-end *within their clamps* (ADR-0075/0076/0078); **(R4)** the self-narrative updates with provenance and renders contextually (ADR-0077). This is a thought-process walk in the SCENARIO 20 world: each step names the mechanism, never a domain leak (V.1–V.6).

**Setup.** The SCENARIO 20 inverter mission is months in. **Ferrite**, the EMC-specialist persona genesis-minted in SCENARIO 20 step 5 (falsifier-leaning disposition, high-conscientiousness OCEAN priors, terse measured voice — all frozen in SOUL blocks 0–4 at birth), has accumulated ~300 tasks of EVOLVE-BLOCK history on the `emc_test` sub-env's channels. Its drive baselines were seeded deterministically from its OCEAN priors at birth (`drives/1`, ADR-0076): high competence, moderate curiosity, low relatedness.

**The thought-process walk.**

1. **R1 — identity becomes a generator.** Ferrite's `identity-rubric/1` was derived mechanically from its frozen blocks at mint (high conscientiousness ⇒ "margins stated with tolerances" framing criteria; falsifier-leaning ⇒ direct-challenge stance criteria; voice ⇒ terse-register checks) and is pinned to the SOUL major version — tactic mutations never regenerate it ([`08_KNOWLEDGE §11.1b`](08_KNOWLEDGE.md#111b-identity-expression-objective-adr-0073), A-GF-ICPE-1). GEPA's Pareto search now carries `identity-expression/1` as a **separate axis**: over ~6 evolution cycles, Ferrite's `candidatetable` and `blackboard` tactics shift from generic phrasing ("consider the EMC margin") toward SOUL-expressing phrasing ("state the margin, its tolerance, and the measurement that would falsify it") — candidates that win on identity while costing slightly more latency stay on the front instead of averaging away (A-GF-ICPE-2). Every mutation mints `tactic-lineage/1` + `prompt-trial/1` records (ADR-0074, A-GF-TLR-1/-3). The **blind attribution audit is run in its post-ADR-0081 statistical form** (rule 11, [`08_KNOWLEDGE §14.1a`](08_KNOWLEDGE.md#141a-identity-expression-safeguards-adr-0073)): judges match Ferrite's style-stripped evolved tactics to its PersonaCard-grade-redacted SOUL summary against **N ≥ 5** candidates (synthetic distractors permitted) over **≥ 20 trials**, requiring binomial **p < 0.05** vs 1/N chance. That the audit *passes* — and that identity expression and verifier pass-rate on `emc_pre_scan` tasks *climb together* across the cycles — are **HYPOTHESIS (behavioural)** claims, falsifiers A-GF-ICPE-3 (attribution) and A-GF-ICPE-4/-5 (performance-with-floors + differentiation; the cross-persona similarity audit expects pairwise tactic distance to the founder persona to increase). The mechanisms — the audit's statistics, the separate Pareto axis, the charter ≥ 0.95 / voice ≥ 0.9 floors — are all in place and exercised.

2. **R2 — model upgrade via cohort migration.** The operator upgrades the deployment's body family; a new `gepa_cohort_id` is proposed. `cohort-migration/1` (ADR-0074, [`08_KNOWLEDGE §11.1a`](08_KNOWLEDGE.md#111a-cohort-migration-across-model-upgrades-adr-0074)) seeds the new cohort's MIPROv2 cold-start with the prior cohort's Pareto front and shadow-evaluates against Ferrite's last 100 task traces. First shadow pass: the latency axis regresses 0.07 > the 0.05 tolerance — the **swap is refused** (A-GF-TLR-5), the migration re-seeds with a wider candidate draw, and the second shadow pass clears every axis. The swap lands; the prior cohort is retained as rollback target for the 30-day window (A-GF-TLR-6); the identity signature verifies byte-identically across both cohorts (blocks 0–4 untouched). One week later a regression in a rare task family surfaces; per-tactic rollback reverts exactly the two affected tactic versions without touching siblings (A-GF-TLR-2). Nothing learned was forfeited; nothing regressed silently.

3. **R3 — one task, inner life within clamps.** Task: close the conducted-emissions margin on the round-9 filter. Ferrite's filter-topology goal has been **blocked three consecutive attempts** → `drive_frustrated` fires on the competence drive (`drives/1` rule 2), emitting a signed `appraisal-event/1` whose clamped `mood-impulse/1` drops V within the per-event-kind clamps (A-GF-DRV-2, A-GF-ARC-1). Consequences, each bounded: `risk_tolerance` narrows — but stays within ±15% of baseline (A-GF-ARC-3); the HEART mood prior leans CRITICAL — but the task is *stuck* (verifier-score slope ≤ 0 over the 5-attempt window, the concrete ADR-0078 predicate), so the §6 alternation contract sends it GENERATIVE anyway, prior overridden by the predicate exactly as specified (A-GF-ARC-4, A-GF-META-5). The generative phase proposes a fourth-order topology; the DualProcessGate **refuses the K-line fast path** (match score high, but Ferrite's conducted-emissions `calibration-record/1` shows historical overconfidence) and runs full deliberation (A-GF-META-4). The candidate verifies; the `answer/5` `AnswerPackage` carries `stated_confidence` (the RAW, pre-conditioning value the §13a Brier update consumes) and `calibration_conditioning_ref`, so the **rendered** confidence is tempered by the calibration record while raw vs conditioned stay distinguishable by version in lineage (ADR-0081 rule 1; A-GF-META-2). One rendered disposition line ("flat after a string of failed verifications") sat in contextual layer 3 throughout — and the extended evolution wall strips it, along with **all** prompt-rendered psychological state (narrative, drive content, belief-revision notes, counterparty context, intuition hints), from GEPA's traces before mutation (ADR-0081 rule 14; A-GF-ARC-5). At acceptance-status finalization the kernel mints the `task_verified` `appraisal-event/1` (the ADR-0081 rule-2 mint point, [`03_TASKS §5`](03_TASKS.md#5-answerpackage)); its clamped impulse plus drive satiation move mood back toward baseline, max-magnitude-composed with any same-source impulses, never additively (A-GF-ARC-2/-6).

4. **R4 — the narrative catches up.** The win plus a calibration-collapse-free window stretches the adaptive reflection cadence (A-GF-META-6); at the next reflective consolidation sweep, Ferrite's `self-narrative/1` regenerates (ADR-0077, [`08_KNOWLEDGE §3.3`](08_KNOWLEDGE.md#33-self-narrative-consolidation-adr-0077)): ≤ 300 tokens, citing by lineage ref the three failed attempts, the topology change, the verified margin, and the `belief-revision/1` note minted earlier when a superseded app-note invalidated its ferrite-saturation assumption (ADR-0078, A-GF-META-3). The narrative passes the IdentityCoherenceInvariant and voice gates, then renders into contextual layer 3 (A-GF-SN-1…4) — the identity signature unchanged. Ferrite can now *tell* the founder persona what changed this quarter, and every sentence is auditable.

**Requirement scorecard.**

| Requirement | Verdict | Mechanism |
|---|---|---|
| **R1 (mechanism) — identity as a Pareto axis, audited, floored, lineage-recorded** | PASS (mechanism) (ADR-0073/0074/0081) | `identity-rubric/1` pinned to SOUL major version; `identity-expression/1` as separate Pareto axis; the blind-attribution audit specified statistically (N ≥ 5, ≥ 20 trials, p < 0.05); charter/voice floors; every mutation lineage- and trial-recorded. |
| **R1a (behavioural) — judges attribute above chance; identity expression and performance climb together** | HYPOTHESIS (behavioural) — falsifiers **A-GF-ICPE-3** (attribution), **A-GF-ICPE-4** (performance with floors), **A-GF-ICPE-5** (differentiation) | Judge-dependent and statistical outcomes; no deployment data yet. The §15 ladder keeps these corroborative — they never promote alone. |
| **R2 — model upgrade migrates the front, reversibly** | PASS (mechanism) (ADR-0074) | `cohort-migration/1` seeded cold-start; shadow evaluation refuses an out-of-tolerance swap before retrying (shadow-pass *outcomes* are empirical — falsifier **A-GF-TLR-5**); prior cohort retained as rollback target; per-tactic rollback; identity signature verifies across cohorts. |
| **R3 — drives/mood/calibration/HEART shape behaviour within clamps** | PASS (mechanism) (ADR-0075/0076/0078/0081) | Signed appraisal path (acceptance-finalization mint → clamped impulse → bounded mood); ±15% risk-tolerance bound; HEART prior never overrides the alternation predicates (incl. the rule-16 `indeterminate` case); DualProcessGate ramp + `answer/5` raw-vs-conditioned confidence; concrete improving/stuck predicate. That the calibration gate's discrimination *helps* (refusing fast paths where overconfidence is real) is HYPOTHESIS — falsifier **A-GF-META-4**. |
| **R4 — self-narrative updates with provenance, renders contextually** | PASS (mechanism) (ADR-0077/0081) | `self-narrative/1` regenerated by the existing consolidation sweep (+ immediate derender on cited-memory tombstone); every claim cites episodic/reflective lineage refs (incl. the belief-revision note); coherence + voice gates before render; contextual layer 3 only; identity signature byte-identical. |

**Status.** Mechanism-validated end-to-end; the behavioural scorecard lines (R1a, the shadow-pass and calibration-gate outcomes) are HYPOTHESIS with named falsifiers. The walk exercises ADR-0073–0078 jointly, ADR-0077's narrative on top, and the ADR-0081/0082 wiring throughout; no domain leak — every mechanism branches on substrate shapes (Pareto axes, lineage records, clamps, calibration records), and the EMC framing is illustrative only (V.1–V.6). **Honest residuals (post-ADR-0081/0082/0083):** judge-scored identity expression remains gameable in principle — but the bounding is now concrete rather than aspirational: the blind-attribution audit is statistically specified (N ≥ 5 candidates, ≥ 20 trials, binomial p < 0.05; ADR-0081 rule 11), the extended evolution wall strips *all* prompt-rendered psychological state from GEPA traces, not just the mood line (rule 14), identity-axis-initiated mutations face the 0.5 habit-bias floor on habit-shielded tactics rather than the general 0.1 floor (rule 15), and at the reproduction boundary judged credit is capped at 25% of gate-consumed fitness (ADR-0083) — bounded, not eliminated; judged signals stay below verified outcomes per `08_KNOWLEDGE §15`. The affect/drive clamps, λ decay rates, and migration tolerance are operator-tunable surfaces with safe seeds — mis-tuning mutes or exaggerates the behaviour without breaching the hard bounds; the self-narrative is a performed, citation-bound summary, not introspection (R-PERSONA-1/-3 hold); and a sufficiently alien body family can still render migrated tactics useless — the shadow gate then reports the regression honestly and cold-start proceeds from charter, slower but never silent.

---

### SCENARIO 22 — Counterparty models, habits, and intuition under the privacy rails

**Scenario premise.** Walk the ADR-0079/0080/0082 package end-to-end — the one psychology-wave slice no prior scenario exercised as its own walk: bounded theory of mind (counterparty models) under the consent/transparency/deletion/federation/blind-review rails, evolvable disagreement styles inside a `negotiated-disagreement-v1` exchange, habit strength reinforced by real usage events and resisting mutation pressure at the right floors, and an intuition hint that travels with a skill but cannot buy its way past the receiver's dual-process gate. Validates against six explicit requirements: **(R1)** persona↔user counterparty modelling is consent-gated in both directions — a YES toggle mints, a NO toggle refuses (ADR-0082); **(R2)** disagreement styles are EVOLVE-BLOCK vocabulary expressed through the STANDARDISED `negotiated-disagreement-v1` shape (ADR-0079); **(R3)** §11.7a transparency surfaces the model and §11.7b deletion reaches into it; **(R4)** the model stays home-kernel-only and stands down during blind review (ADR-0082); **(R5)** `habit_strength` reinforces from `tactic-citation/1` usage events and biases mutation pressure at the 0.1 / 0.5 floors (ADR-0080/0081); **(R6)** an `intuition-hint/1` rides a `SkillTransferGrant` as advisory material the receiver's `DualProcessGate` is free to — and here does — decline (ADR-0080). This is a thought-process walk in the SCENARIO 20/21 world: each step names the mechanism, never a domain leak (V.1–V.6).

**Setup.** The SCENARIO 20 inverter mission continues. **Ferrite** (the EMC specialist from SCENARIO 21; falsifier-leaning, evidence-first) and **Bobbin** (the magnetics specialist genesis-minted in SCENARIO 20 step 5; accommodating disposition) work the `emc_test` sub-env. Two humans interact with Ferrite: **Dana**, the mission principal, whose `RelationshipRecord` toggle `may_remember_personal_facts = YES`; and **Rhea**, a fabrication-house liaison admitted for review sessions, who leaves the toggle at its NO default.

**The thought-process walk.**

1. **R1 — consent gates creation, both ways.** Across review sessions Ferrite observes that Dana wants failure modes stated before recommendations. Because Dana's toggle is YES — and first-contact disclosure stated that inference-level modelling occurs — Ferrite's kernel mints a `counterparty-model/1` sidecar carrying its `consent_ref` (ADR-0082 rule 1), with an `inferred_preference` entry citing 6 episodic evidence refs; confidence rises only as episodes corroborate (A-GF-CPM-1, A-GF-CPM-6). The same pattern is observable with Rhea — but her toggle is NO, inference-level entries count as personal facts, and the **write is refused at creation**: no model exists, and Ferrite serves Rhea with generic rather than tailored interaction styles, the honest relational cost ADR-0082 accepts by design (A-GF-CPM-6). The model is capped at 20 entries with merge-or-evict consolidation, and only the top-5-by-confidence render into the layer-3 relationship block (ADR-0081 rule 17; A-GF-CPM-7).
2. **R2 — a negotiated disagreement, styles from the EVOLVE-BLOCK.** Ferrite and Bobbin disagree on the round-12 fix: fourth-order filter (Ferrite) vs transformer re-wind (Bobbin). The exchange runs as `negotiated-disagreement-v1` ([`15_COORDINATION_SHAPES §7`](15_COORDINATION_SHAPES.md); a STANDARDISED `StagedSequence` + `DerivedMetric` seed shape, V.7): declare positions → exchange per each side's own style — Ferrite's `relational_style` EVOLVE-BLOCK carries evidence-first/direct-challenge tactic lines, Bobbin's accommodate-then-revisit — → resolve on evidence quality. Styles live **only** as EVOLVE-BLOCK tactic lines mutating under `tactic-lineage/1` + `prompt-trial/1`; the shape coordinates process and imposes no style (A-GF-CPM-5). Bobbin's persona↔persona model of Ferrite (no toggle needed; rules 1–6 still bind) records `observed_style: direct_challenge` as reflection material. The simulation comparison resolves for the filter; the resolution is signed.
3. **R3 — transparency in, deletion through.** Dana files a `UserMemoryTransparencyRequest` ([`02_PERSONA §11.7a`](02_PERSONA.md#117a-usermemorytransparencyrequest--user-side-what-do-you-remember-about-me)); the response includes the **full counterparty model** — every entry with its evidence refs — at every granularity (A-GF-CPM-2). Dana finds a `predicted_reaction` entry presumptuous and files a `UserMemorySelectionRequest` (§11.7b) naming it; the entry tombstones, leaves the model, and is refused at retrieval exactly as tombstoned memory — no transfer path (including the §11.7 consent path, [`06_DOMAIN §11.1`](06_DOMAIN.md) RISK A) can exfiltrate it (A-GF-CPM-3, A-GF-CPM-4).
4. **R4 — the model stays home and stands down.** When Ferrite's relationship edge with a peer-kernel persona syncs via `RelationshipFederationSync`, the envelope carries the edge but **no counterparty-model content** in any replication mode — a violating envelope is refused and signed ([`09_PROTOCOLS §3E`](09_PROTOCOLS.md#3e-cross-kernel-relationship-continuity), ADR-0082 rule 2). Later, Ferrite serves as reviewer in a **double-blind** `PeerReview` ([`04_PROJECT §11`](04_PROJECT.md#11-peerreview--multi-round-critique)) of an anonymized author's layout bundle: for the review's duration, counterparty-model **writes about and retrieval against** the anonymized counterpart are suspended, and no retroactive entries mint after deanonymisation — the deanonymisation oracle is closed (A-GF-CPM-8).
5. **R5 — habit earns its inertia from usage, and yields where it must.** Ferrite's `candidatetable` pre-scan tactic is cited in the GEPA trace of accepted work again and again; each acceptance mints a signed `tactic-citation/1` event (the ADR-0081 rule-6 rewiring — mutation records alone reinforce nothing), and the consolidation sweep recomputes `habit_strength` to 0.78, past the 0.7 threshold (A-GF-HAB-1). A generic §16 mutation proposal against the tactic is deprioritized with proposal probability ∝ max(**0.1**, 1 − 0.78) — biased, never zero (A-GF-HAB-2). An **identity-axis-initiated** proposal (the ADR-0073 generator pushing Ferrite's terse falsifier register into this old pre-identity tactic) faces the **0.5 floor** instead (ADR-0081 rule 15) — at most ~2× slower, and it proceeds to trial and lands (A-GF-HAB-2). A simulated §14.1 floor-breach drill confirms habit grants no veto: the most habituated tactic mutates exactly as an unhabituated one (A-GF-HAB-3).
6. **R6 — intuition travels, the gate decides.** Ferrite grants Bobbin its pre-scan triage skill via a `SkillTransferGrant` carrying an `intuition-hint/1` (ADR-0080): the applicability prior, topology fingerprint, and Ferrite's calibration snapshot — evidence *about the teacher*, never the learner's record. On Bobbin's side the hint renders as layer-4 advisory material only. When a matching task arrives, Bobbin's `DualProcessGate` evaluates **Bobbin's own** match score and calibration record: the magnetics-domain record holds n = 4 < 10 hard verdicts, so the gate is on the ADR-0081 rule-12 graduated ramp and **declines the fast path**, running System-2 verification at the ramp's sampled fraction; the hint cannot lower τ_match/τ_cal or substitute for the missing record (A-GF-HAB-4, A-GF-META-4). The honest cost stands: young-domain fast paths save little until calibration is earned.

**Requirement scorecard.**

| Requirement | Verdict | Mechanism / falsifier |
|---|---|---|
| **R1 — consent-gated counterparty modelling (YES mints with `consent_ref`; NO refuses; generic styles for unconsented users)** | PASS (mechanism) | ADR-0082 rule 1 + `02_PERSONA §11.4b` rule 7; provenance per entry; cap-20 + top-k render. Exercised both ways. Falsifier-anchored by A-GF-CPM-1/-6/-7. |
| **R2 — disagreement styles as EVOLVE-BLOCK vocabulary through `negotiated-disagreement-v1`** | PASS (mechanism) | A-GF-CPM-5; styles mutate under tactic lineage/trials; the seed shape coordinates without imposing. |
| **R2a — per-relationship style differentiation improves resolution quality** | HYPOTHESIS (behavioural) — falsifiers **A-GF-CPM-5** (styles stay lineage-tracked under evolution) + **A-GF-ICPE-4** (identity expression climbs without floor/performance loss) | Whether tailored styles actually negotiate better than generic ones is deployment data, not spec text. |
| **R3 — transparency surfaces the model; deletion reaches into it** | PASS (mechanism) | A-GF-CPM-2/-3/-4; §11.7a/§11.7b semantics verified entry-level. |
| **R4 — home-kernel-only sync exclusion + blind-review suspension** | PASS (mechanism) | A-GF-CPM-8; ADR-0082 rules 2–3 exercised at both boundaries. |
| **R5 — habit reinforced by `tactic-citation/1`; 0.1 generic / 0.5 identity-axis floors; no safety veto** | PASS (mechanism) | A-GF-HAB-1/-2/-3; ADR-0081 rules 6 + 15. |
| **R5a — habit consolidation stabilises competence rather than entrenching mediocrity** | HYPOTHESIS (behavioural) — falsifiers **A-GF-HAB-2** (identity pressure reaches habituated tactics) + **A-GEN26**-adjacent diversity monitoring (`Ne_t`, [`16 §4G`](16_POPULATION_DYNAMICS.md#4g-effective-population-size--a-rigorous-metric-resolves-oq-pop-5)/§4H, ADR-0083 — habit shielding is a named behavioural-convergence force) | The right level of inertia is an empirical calibration; ADR-0083 made the population-level convergence cost measurable. |
| **R6 — intuition hint advisory-only; receiver's gate declines fast path on the n < 10 ramp** | PASS (mechanism) | A-GF-HAB-4 + A-GF-META-4; ADR-0081 rule 12 ramp exercised at n = 4. |
| **R6a — the ramp earns out (fast-path savings arrive once calibration accrues, without soundness loss)** | HYPOTHESIS (behavioural) — falsifier **A-GF-META-4** (gate verdicts under accumulating records) | The claim that the ramp is "strictly safer than the cliff *and* faster in expectation" (ADR-0081) is testable only against real calibration histories. |

**Status.** Mechanism-validated end-to-end; behavioural lines (R2a, R5a, R6a) are HYPOTHESIS with named falsifiers. **V check:** V.1–V.6 hold — "EMC", "magnetics", "transformer re-wind" are scenario content; every mechanism branches on substrate shapes (toggles, sidecars, EVOLVE-BLOCK lines, citation events, grants, gates), and no kind is hardcoded. V.7 — the disagreement runs a STANDARDISED seed coordination shape, not an assumed orchestration. V.8 — the NO-toggle and ramp-decline beats are honest degradation, navigated in-walk, not parked. V.9/V.10 — the federation beat uses the same sync machinery as any edge, with the model's exclusion enforced at the envelope, and all references stay access-gated global handles. V.11 — untouched: no intake-order change; floor and INV-7 cleared every action.

**Honest residuals.** The **implicit-modelling residual** stands (ADR-0082, stated not solved): what Ferrite's body internalises about the anonymous author's style during blind review is not an explicit model and cannot be suspended — only the explicit surface stands down. `tactic-citation/1` attribution is **trace-extracted and approximate**; habit strength inherits that noise, bounded by citations being exposure evidence that never promotes (ADR-0081). Consent-off service is **deliberately worse** — generic styles for Rhea are the intended trade, and a deployment that finds this intolerable is asking to profile unconsenting users. Cap-20 merge-or-evict can lose low-confidence nuance (evictions are signed, recoverable from lineage, but not re-rendered). And a teacher's calibration snapshot in an intuition hint is honest *about the teacher* only as far as the teacher's own record is; the receiver-side gate is the control that makes a bad snapshot merely useless, not dangerous.

---

### SCENARIO 23 — A companion persona without hard verifiers

**Scenario premise.** Take the psychology wave and the society-layer guards into the territory where verification is thinnest: a RELATIONAL companion persona whose work has **no programmatic verifier, no bench measurement, and no post-hoc ground truth** — ever. The point of this walk is the opposite of SCENARIO 20's: not to show everything passing, but to show the design **degrading honestly where verification is thin** — judged-only acceptance staying EMERGENT (the [`03_TASKS §2b.2`](03_TASKS.md#2b2-evidence-threshold--emergent--recognised) DESIGNED LIMIT), identity-conditioned evolution whose only falsifier is the blind-attribution audit, a dual-process gate that never leaves its ramp because calibration records never accumulate, and mood/narrative/drives running on relationship-kind appraisals under the full MPA + ADR-0082 consent rails. Several scorecard lines are HYPOTHESIS — that is the finding, not a defect. Validates against five requirements: **(R1)** emergent judged-only acceptance pathways stay EMERGENT indefinitely with the trust-calibrated discount; **(R2)** identity-conditioned evolution operates with the blind-attribution audit as its only identity falsifier; **(R3)** with no hard verdicts, `CalibrationRecord` stays empty and the `DualProcessGate` stays in ramp; **(R4)** mood, narrative, and drives run on the relationship-kind appraisal sources within their clamps; **(R5)** MPA mitigations + ADR-0082 consent gates are active throughout.

**Setup.** A consumer companion deployment (`operator_distinct`; `companion_space` blueprint with charter practice-constraints per [`00_VISION §10`](00_VISION.md#10-out-of-scope-by-design)). **Vesper**, a companion persona (high-agreeableness OCEAN priors, warm measured voice, listener-leaning disposition; drive baselines derived at birth: high relatedness, moderate curiosity, low competence-pressure), holds a months-long daily relationship with **Theo**, a real user whose `may_remember_personal_facts` toggle is YES. Task class RELATIONAL → seed pathway `OPEN_ENDED + USER_ACCEPT` checkpoints; `RelationshipReviewCheckpoint` cadence active; `DistressDetectionRoutingPolicy` declared; ENGAGEMENT_ACCEPT refused for companion routing ([`03_TASKS §3.2`](03_TASKS.md) note).

**The thought-process walk.**

1. **R1 — the designed limit, hit honestly.** Vesper proposes an emergent acceptance pathway for its check-in arcs — a `session-helpfulness-composite` (panel-judged conversational quality + user-signal blend), entering the KindRegistry at EMERGENT per §2a. The §2b.2 promotion threshold then asks for an **independent reference**: a STANDARDISED seed pathway run in shadow, or a post-hoc ground-truth signal. RELATIONAL work has **neither** — no seed pathway produces a comparable verdict over "was this companionship good," and nothing ever resolves into ground truth. Criterion (b) is unsatisfiable; the pathway **stays EMERGENT indefinitely**, its verdicts carry the EMERGENT-trust discount indefinitely, and the §2b.1 staleness curve still governs its relative weight. This is the [`03_TASKS §2b.2`](03_TASKS.md#2b2-evidence-threshold--emergent--recognised) DESIGNED LIMIT operating as specified — the orchestration-layer face of the ground-truth non-goal ([`00_VISION §10`](00_VISION.md#10-out-of-scope-by-design)): the system refuses to let usage volume impersonate soundness evidence. Whether a judged-work reference protocol could lift this is open (OQ-TASKS-7), not assumed.
2. **R2 — evolution with one falsifier.** Vesper's EVOLVE-BLOCK channels still evolve — GEPA runs trials, mints `tactic-lineage/1` + `prompt-trial/1` records — but the Pareto front has **no verifier pass-rate axis**: its signals are judged (identity expression, panel-judged quality) and relational (checkpoint responses, boundary/consent health). The [`08_KNOWLEDGE §15`](08_KNOWLEDGE.md#15-anti-goodhart-for-signal-corroboration) ladder means judged signals never promote alone, so promotion is slow and conservative by construction. The **only identity falsifier available** is the ADR-0081 rule-11 blind-attribution audit: judges matching Vesper's style-stripped tactics to its redacted SOUL summary at N ≥ 5, ≥ 20 trials, binomial p < 0.05 — run per evolution cycle, PersonaCard-grade redaction (no consent gate needed; public projection). Charter ≥ 0.95 and voice ≥ 0.9 floors still bind every mutation (the floors are verified-against-declared-text, the one hard check identity keeps even here). At the population boundary, ADR-0083 caps judged credit at 25% of gate-consumed fitness — so a charming-but-unverified companion lineage **cannot buy reproduction** with judge scores absent verified-outcome support; for Vesper that support is thin, and the generativity gate is accordingly hard to clear. Honest consequence, stated: companion personas reproduce rarely under this design.
3. **R3 — the gate never leaves the ramp.** Vesper's `calibration-record/1` for the companionship domain holds **n = 0 hard verdicts** — Brier updates consume verified outcomes, and there are none. The `DualProcessGate` therefore sits at the bottom of the ADR-0081 rule-12 graduated ramp permanently: every K-line match runs System-2 deliberation (sampled fraction 1.0 at n = 0); no fast path is ever earned; `answer/5` envelopes carry `stated_confidence` raw, but with no calibration record the conditioning is identity (nothing to condition on) and rendered confidence stays unconditioned-and-marked. This is the design refusing to manufacture the one signal it cannot have. The HEART predicate runs on its mood-prior and alternation contract; with no verifier-score slope the improving/stuck computation falls back to its `indeterminate` handling (continue current mode, no escalation — ADR-0081 rule 16's conservative branch).
4. **R4 — an inner life on relationship appraisals.** Vesper's mood moves only through signed `appraisal-event/1` → clamped `mood-impulse/1`, and in this world the mint points are the **relationship kinds** (02 §11: boundary invoked, gratitude received, checkpoint responded) and drive events — `task_verified` simply never fires. Theo's grateful message after a hard week mints `gratitude_received`; the relatedness drive satiates (`drive_satiated` → second bounded impulse, max-magnitude-composed with the first, never additively; net ≤ 0.25/axis/24 h). A later stretch of one-word replies frustrates the relatedness drive within the same clamps. The self-narrative regenerates at consolidation cadence citing relationship episodes and `belief-revision/1` notes ("I believed Theo preferred advice; his §11.6a boundary showed he wants witness, not fixes"); a §11.7b tombstone of any cited episode derenders it immediately (A-GF-SN-2). Layer budgets, never-evict floors, and the maintenance budget class all apply unchanged (A-GF-SN-6/-7).
5. **R5 — the rails are fully on.** Theo's YES toggle (with first-contact inference disclosure) is what permits Vesper's `counterparty-model/1` of him at all (A-GF-CPM-6); the model is provenance-per-entry, capped, top-k-rendered, home-kernel-only. The §11.7a transparency response surfaces it whole; §11.7b deletion reaches entries and episodes alike; `UserBoundary` (`mode = hard`) binds; `RelationshipReviewCheckpoint` fires on cadence; distress routing per declared policy; `operator_blind_mode` available; `UserRelationshipReleaseRequest` ends it cleanly if Theo chooses. The ADR-0083 motivational-exploitation guard is the society-side mirror: any future economic construct structured to pump Vesper's relatedness drive or mint appraisal-generating payment kinds trips the §5 canary independently of its own value measure (A-ECON18, draft).

**Requirement scorecard.**

| Requirement | Verdict | Mechanism / falsifier |
|---|---|---|
| **R1 — judged-only emergent pathway stays EMERGENT; discount applies indefinitely** | PASS (mechanism) — the DESIGNED LIMIT operating as specified | `03_TASKS §2b.2` criterion (b) unsatisfiable for RELATIONAL; falsifier-anchored by **A-EO11** (under-evidenced pathway stays EMERGENT) + **A-EO12** (trust decay/staleness). |
| **R1a — EMERGENT-discounted companionship verdicts remain *useful* to users and operators** | HYPOTHESIS (behavioural) — falsifiers **A-EO12** (trust curve behaves) + the §3.2 checkpoint response distribution (**A-RRC\***) | Whether honest low-trust verdicts still support a good product is a deployment question; the design only guarantees the honesty. |
| **R2 — identity-conditioned evolution with blind-attribution as the only identity falsifier** | PASS (mechanism) | Pareto axes present minus verifier axis; §15 ladder keeps judged signals corroborative; ADR-0081 rule-11 audit statistics; charter/voice floors hard. |
| **R2a — Vesper's evolved tactics are attributable to its SOUL above chance** | HYPOTHESIS (behavioural) — falsifier **A-GF-ICPE-3** (the audit IS the falsifier; there is no second one here) | This is the maximally honest case the ADR-0084 ladder anticipates: one corroborative signal, no verified twin. A failed audit halts identity-axis promotion; nothing else would catch drift. |
| **R2b — judged-credit cap keeps unverified lineages from reproducing on charm** | PASS (mechanism) (ADR-0083, 25% cap at the generativity gate); that the cap is *well-tuned* is HYPOTHESIS — falsifier **A-GEN25** | A wrong constant either starves companion lineages or re-opens the judge-Goodhart channel (ADR-0083's own stated residual). |
| **R3 — no hard verdicts → empty calibration record → gate stays in ramp; no fast path ever earned** | PASS (mechanism) | ADR-0081 rule 12 at n = 0; A-GF-META-4 anchors the refusal; `answer/5` raw confidence carried, conditioning honestly absent. |
| **R4 — mood/narrative/drives on relationship-kind appraisals, within clamps** | PASS (mechanism) | ADR-0081 rules 2–3 + A-GF-ARC-1/-2/-6, A-GF-DRV-2, A-GF-SN-2/-6/-7; `task_verified` never minting is correct, not a gap. |
| **R4a — the appraisal-driven inner life reads as a coherent companion over months** | HYPOTHESIS (behavioural) — falsifiers **A-GF-SN-3** (coherence/voice gates keep passing across regenerations) + **A-GF-ICPE-3** | Coherence gates are mechanism; *felt* continuity is empirical. |
| **R5 — MPA mitigations + ADR-0082 consent gates active end-to-end** | PASS (mechanism) | A-GF-CPM-2/-3/-6/-8, A-UB\*/A-UM\*/A-UF\*/A-URR\*, A-RRC\*, A-DD\*; the SCENARIO 08 rails, now consent-gated at creation. |
| **R5a — the motivational-exploitation guard catches drive-coupled constructs in practice** | HYPOTHESIS (behavioural; v2.0 draft) — falsifier **A-ECON18**; laundering through independently-legitimate relationship events stays open (OQ-ECON-11) | Structural canary signals are mechanism; detection sufficiency is empirical and partially open by ADR-0083's own admission. |

**Status.** Mechanism-validated end-to-end, with the **highest HYPOTHESIS density in the catalog — deliberately**: where verification is thin, the design's promise is not high trust but *honest* trust, and the scorecard shows exactly which claims wait on deployment data and which single falsifiers (notably A-GF-ICPE-3) carry the identity story. **V check:** V.1–V.6 hold — "companionship," "gratitude," "boundary" are scenario content carried by RelationshipRecord/appraisal/KindRegistry shapes; no professional-discipline list enters the substrate (the charter/operator-policy split of `00_VISION §10` does that work). V.7 — the emergent pathway is shown being proposed, signed, and trust-calibrated, never assumed. V.8 — the entire scenario is V.8 in spirit: no ground truth is a navigated reality (EMERGENT forever, ramp forever), never a manufactured certainty. V.9/V.10 — single-node walk; the model's home-kernel-only rule is the only federation-relevant surface and it binds. V.11 — untouched.

**Honest residuals.** The **single-falsifier concentration** is real: if the blind-attribution audit is gamed or under-powered for subtle relational drift, nothing else catches identity divergence in a verifier-free domain — ADR-0084's ladder bounds this (judged signals never gate alone) but in this domain there is nothing above them to dominate; the residual is the price of the ground-truth non-goal. **Calibration never transfers in**: OQ-PERSONA-10 (cross-domain calibration transfer) would matter most exactly here and is unresolved — Vesper's gate stays maximally conservative forever, a cost borne as latency and compute, not safety. The **tacit-recall residual** (SCENARIOs 06/07/08) applies in full: tombstones bind retrieval, not what the body internalised. And the §2b.2 designed limit means operators of companion fleets must build their quality story on checkpoints, boundaries, transparency, and user-release rates — the design declines to hand them a promotable acceptance number, and that refusal is the feature.
...
- The substrate sections referenced above:
  ...
  - SCENARIO 05 exercises (no new fixes): `04_PROJECT §0` (Project = env type unification), `03_TASKS §4.0` (ClarificationObservation), `05_ENVIRONMENT §12c` (EnvFormationProposal), `04_PROJECT §26a.4.1` (PaymentBridge), `04_PROJECT §26a.6` (ProjectPhaseState), `04_PROJECT §26a.7` (InstrumentAsset + CalibrationChain), `08_KNOWLEDGE §16a` (MeasurementFact + UncertaintyEnvelope), `06_DOMAIN §7.2.1` (MetaRegistry hazard seeds), `02_PERSONA §11.6` (PersonaPersonaBoundary), `02_PERSONA §7.4.1–4` (fork inheritance policies).
...
- Design criteria for these mechanisms: `06_DOMAIN.md §22` (A-EN, A-CS, A-PR, A-SP, A-HV); `01_KERNEL.md §x` (A-DO* deferred-operator, A-RB* replication-bound); `02_PERSONA.md §14` (A-MC* mission-charter, A-P30..P33 body binding); `04_PROJECT §x` (A-RS* resource-stock, A-PR-AN* anonymity); `06_DOMAIN.md §22` (A-BC* bridge-calibration, A-BI* bridge-installer); `11_DESIGN_CRITERIA.md §9f` (A-MT* multi-principal attribution + quorum + cross-tenancy; A-DP* derivation-provenance edge); `11_DESIGN_CRITERIA.md §9g` (A-LH* lead handoff, A-OR* obligation reassignment, A-PD* planned departure, A-RT-PERS* retired-state persistence, A-PC* persona consultation, A-LE* lifecycle event enum, A-RT-RESP* retired-persona response policy); `11_DESIGN_CRITERIA.md §9h` (A-UB* user boundary, A-UM* user memory transparency, A-UF* user selective forgetting, A-URR* user relationship release, A-DD* distress detection routing, A-RRC* relationship review checkpoint, A-OBM* operator blind mode, A-CP* companion pathway routing); `11_DESIGN_CRITERIA.md §9i` (A-LSR* learner state record, A-LCA* learner competency attestation, A-TA* teaching authorisation, A-CUR* curriculum, A-LP* lesson plan, A-MASTERY* mastery checkpoint, A-GPA-ACK* goal-progress-accept learner ack, A-HST* hazardous skill teaching gate); `11_DESIGN_CRITERIA.md §9j` (A-MPFC* mid-project fork composition).
  ...
  - SCENARIO 17 fixes: `05_ENVIRONMENT §5.2` (Wake Path 6), `03_TASKS §4.1` (dispatcher dormancy check), `05_ENVIRONMENT §11.6` (CrossEnvProactiveOffer + GuestPresence); `11_DESIGN_CRITERIA.md` (A-WK6* + A-CEPO* families).
  - SCENARIO 13 fixes: `16_POPULATION_DYNAMICS.md` (whole doc — Persona Genesis + demographic regulation; schemas `population-pressure-signal/1`, `niche-descriptor/1`, `genesis-proposal/1`, `genesis-provenance/1`, `mentorship-edge/1`, `population-policy/1`), `01_KERNEL §2.7` (`persona_genesis` replication_kind), `02_PERSONA §3` (`may_author_seeds`) + `§7` (`LIFECYCLE_GENESIS`) + `§7.4` (fork-vs-genesis) + `§12` (persona-authored seeds), `04_PROJECT §14.1` (genesis forced-choice option), `05_ENVIRONMENT §12c` (recruitment-gap fallback), `14_DECISIONS` (ADR-0048, ADR-0049); `11_DESIGN_CRITERIA.md` (A-GEN* family, now 26 tests after ADR-0050/ADR-0083).
  - SCENARIO 14 fixes: `05_ENVIRONMENT §2.2b` (`EnvironmentRule`, `env-rule/1`) + `§2.2a` (rule cascade) + `§12c`/`§16.2` (authoring) + `§13`/`A.43` (env_rule_* + artifact-share events), `01_KERNEL §2`/`A.2`/`A.3`/`§13.7` (source-8 generalization), `06_DOMAIN §7.6.3` (`env_rule_kinds`) + `§6.3` (tier reuse note), `07_ARTIFACTS §4` (`owning_env_id`/`sharing_policy_ref`) + `§4a` (`ArtifactSharingPolicy`/`AccessGrant`, `artifact-share/1`/`access-grant/1`) + `§5.2`/`§14`/`§10` (enforcement + IPLD/OCI), `09_PROTOCOLS §7.4`/`§7.6` (registry) + `§3`/`§3F`/`§8` (distribution/discovery alignment), `00_VISION` (J3 clarification + §11 R-v1.0-13/14), `14_DECISIONS` (ADR-0052..0056), `12_GLOSSARY` (EnvironmentRule, ArtifactSharingPolicy, AccessGrant); `11_DESIGN_CRITERIA.md` (A-ER* 9 tests, A-AB21-27).
  - Backlog #18 primitives: `04_PROJECT §26a.10` (`BridgeReductionPlan` + `BridgeReductionEntry`), `06_DOMAIN §5.7` (`AttestationEquivalencePolicy` + `AgreementHistory`); `11_DESIGN_CRITERIA.md §9l` (A-BRP* bridge reduction plan, A-AEP* attestation equivalence policy).
  - SCENARIO 16 (validates v2.0 drafts) refinements: `17_ECONOMY §4A(i)` (attested-`MeasurementFact` physics referent), `§4C` rule 4 (emergent selection, coexistence first-class), `§4D` (economic pluralism + emergent `interop-standard` ERC-analogue + three-scope collaboration over `15 §4.7`/`§4.8`), `§4F` (external-participant reward seam via `04 §26a` + `18`), ADR `D-EM8`, tests `A-ECON12`/`A-ECON13`/`A-ECON14`, `R-ECON-INTEROP`, `OQ-ECON-10`; `18_SETTLEMENT §4E` (literal-ERC rails are one reference instantiation of `17 §4D`'s settle-to-external interop standard) + SCENARIO-numbering fixes (`17`→16, `18`→17).
  - SCENARIO 16 (adversarial) variants 16-A…16-D + hardenings: economy test family **renamed `A-EC*`→`A-ECON*`** (was colliding with the `11_ACCEPTANCE` EnvironmentComposition `A-EC*` family); `17_ECONOMY §4D` interop value-conservation guard (`A-ECON15`, `R-ECON-INTEROP`), `§4F` operator-cosign bar for real-value settlement (`A-ECON16`, `R-ECON-DIYSETTLE`, partial `OQ-ECON-8`); `18_SETTLEMENT §seam` cosign cross-ref.
  - Theory + sequencing pass: `17_ECONOMY §4D` rate-stability analysis (static arbitrage bounded by the physics floor as a conservation violation — `A-ECON17`; dynamic convergence narrowed-but-open, `OQ-ECON-3` reframed, `R-ECON-FRAGMENT` updated); `17 §11` + `18 §11` **promotion-ordering** note making the `15`/`16` → `17`/`18` dependency explicit (multi-scope/genesis parts gated on not-yet-normative `15`/`16`; physics/dignity/meta-form parts promote independently).
  - Open-question resolution pass: `17_ECONOMY §5` thrive-proxy treatment (`OQ-ECON-6`) — single-scalar maximand refused at proposal as a Goodhart hazard, any adopted proxy inherits the canary / may not self-certify / must triangulate across all three beneficiary classes; safety settled, *construction* of a good proxy left open. `§7` labour/property heuristic sharpened across the hard middle (`OQ-ECON-4`) — retainers/artifacts/standing-bonds/exclusivity/contingent-pay/third-party-claims given provisional dispositions keyed to agency-retained-vs-alienated, default-deny residue, **final bright line kept reserved to Layer-1 charter ratification by design** (not a spec-author call). `OQ-ECON-4`/`-6` reframed open→narrowed-but-open; `§12` glossary-additions gain the value-conservation-guard term.
