...
| OQ-ENV-3 | Joined-env federation CRDT partition recovery: when the network heals after partition, which conflicts auto-resolve vs require signed branch selection? Currently `CONFLICT_PARKED` preserves the divergent branches while personas or peer environments reconcile, choose a signed branch, or seek optional operator evidence. Unrelated state and work continue; operator-only authority gates only its exact transition. | Federation WG | v1.1 partition recovery policy. |
