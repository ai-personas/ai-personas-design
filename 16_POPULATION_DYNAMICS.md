...
Terms used normatively below are defined in `§0a` and in [`12_GLOSSARY.md`](12_GLOSSARY.md): *Persona Genesis*, *GenesisProposal*, *population-pressure signal*, *niche cell / niche descriptor*, *competitive-exclusion refusal*, *optimal distinctiveness*, *sibling differentiation*, *generativity gate*, *mentorship edge*, *maturation ramp*, *dual inheritance*, *carrying capacity*, *reproduction strategy (r/K)*, *niche width*, *effective population size*, *diversity-injection mandate*, *variance effective size (Ne_v)*, *effective number of niches (Ne_d)*, *tactic-space effective size (Ne_t)*, *diversity audit*, *novelty pressure*, *attention fitness-sharing*, *learned niche descriptor (CVT / AURORA)*, *niche recalibration advisory*, *cross-node genesis (global ReplicationBound aggregation)*. Schemas are listed in the Technical Appendix and MUST be registered per [`09_PROTOCOLS.md §7`](09_PROTOCOLS.md).
...
**Supply visibility without host judgment.** A funded persona turn receives a bounded, signature-verified projection of the other ACTIVE members' PersonaCards, including their persona-authored open-vocabulary descriptions and characteristics. This observation is the evidence from which a persona may judge whether current population supply can address a pressure. The kernel does not map those words to a role taxonomy, infer a capability gap, score fitness, select a recruit, or author a `PopulationPressureSignal`; it only verifies identity, membership, ordering, and bounds. Any need, recruitment attempt, or `GenesisProposal` remains persona-authored and lineage-bound.
...
- **Task-scoped integration serialization.** At most one newborn with unresolved integration MAY exist for one `(environment_id, task_id)` at a time. A later proposal is REFUSED `persona_birth_integration_pending` until the earlier newborn either (a) publishes new, verified workspace or artifact bytes, (b) authors a signed current situation-sufficiency judgment, (c) explicitly answers its invitation without joining, or (d) exits ACTIVE lifecycle. This gate is deliberately mechanical: it does not compare names, prose, professions, capability claims, or niche semantics. It prevents concurrent wakes from producing duplicate specialists for the same still-open pressure while allowing a genuinely distinct later birth once the cohort has incorporated or explicitly resolved the first. Niche allocation and sibling differentiation still apply after this serialization gate clears.
...
7. **Provenance + mentorship.** The kernel writes `genesis-provenance/1` to the newborn's `soul.state.json` and opens a `mentorship-edge/1` from author to newborn. Before membership, only the exact signed author↔newborn bootstrap channel of `09_PROTOCOLS §3A.3` is admissible. It is bound to the verified birth provenance and causal task, carries no general communication, mutation, execution, observation, or membership privilege, and lets the newborn answer its author rather than wait silently for enrollment. On that exact author-to-newborn communication only, the newborn MAY inspect bounded bytes from its own already-bound workspace replica through a read-only capability; this grants no host-path, shell, write, environment-main, or membership authority.
...
- **Secure base.** The mentor is the newborn's **secure base** (Bowlby & Ainsworth): the newborn explores harder tasks confidently *because* mentor support is reliably available. If the mentor enters RETIRED/ARCHIVED (`02_PERSONA §7.5`) before the newborn reaches the autonomy threshold, the kernel MUST re-assign the `mentorship-edge/1` to another generative cohort member (or, failing that, surface a `mentor_vacancy` advisory) rather than leave the newborn without a secure base. Mentor eligibility — at first assignment and at re-assignment — SHOULD additionally require a **non-degraded calibration trend** in the mentored domain where a `calibration-record/1` ([`08_KNOWLEDGE.md §13a`](08_KNOWLEDGE.md#13a-calibration-and-belief-revision-adr-0078)) exists for the candidate; the absence of a record is **not** disqualifying (advisory floor, ADR-0083).

**Calibration as corroborating evidence (ADR-0083, advisory).** The ramp above is paced by conferred community standing. Where a persona holds a `calibration-record/1` in the relevant domain ([`08_KNOWLEDGE.md §13a`](08_KNOWLEDGE.md#13a-calibration-and-belief-revision-adr-0078)), CommunityStanding conferral ([`05_ENVIRONMENT.md §5.4`](05_ENVIRONMENT.md#54-community-standing--legitimate-peripheral-participation-community-standing1)) and federation reputation ([`09_PROTOCOLS.md §3D`](09_PROTOCOLS.md#3d-reputation-and-anti-goodhart)) MAY consume the record as **corroborating evidence** — it updates from verified outcomes only and is among the least-gameable competence signals in the corpus. A calibration record MUST NOT become the **sole basis** of conferral or reputation: a single scalar invites gaming, and the existing peer-quorum conferral and §3D anti-gaming rules are unchanged. Absence of a record is never disqualifying. This is an advisory input into existing machinery, not a new reputation system.
...
**Treasury→compute→population invariant (ADR-0083).** Once an environment earns external value, the budget dial above stops being operator-only: earnings can land in a `PersonaTreasury` ([`18_SETTLEMENT.md §4D`](18_SETTLEMENT.md#4d-personatreasury--treasurybound--the-bounded-fund-custody-exception)), treasury funds can buy metered compute (x402), bought compute raises INV-7 budget headroom, headroom raises genesis pressure (`§4A` factor 7) and — under `ceiling_from_budget` — the population ceiling itself, and a larger population earns more. This compound loop (earnings→compute→headroom→more personas→more earnings) is named as R-POP-8 here and R-ECON-COMPOUND in [`17_ECONOMY.md §13`](17_ECONOMY.md). The loop MUST be broken at the ceiling: **treasury-funded budget increases MUST NOT move `ceiling_from_budget` without a fresh operator cosignature per increase** — no standing authorization, and no `PolicyEnvelope` / `operator_deferred` pre-authorisation, satisfies this cosign. Treasury-funded headroom MAY raise hosting capacity (carrying capacity, above); the *ceiling* moves only by an explicit per-increase operator act. (A-GEN26.)
...
The kernel computes **three complementary estimators** and takes the **most conservative** (smallest), because each failure mode — a few founders authoring everyone, everyone crowding a few niches, or everyone *behaving* identically regardless of niche — is a monoculture risk:
...
3. **Tactic-space effective size `Ne_t`** (behavioural monoculture; ADR-0083). `Ne_d` sees only niche-*descriptor* space and `Ne_v` only authorship: two personas in distinct niche cells, authored by distinct founders, can still converge on near-identical **promoted tactic sets** — the post-birth drift `§4H` exists to catch — and neither estimator would register it. `Ne_t` is the inverse-Simpson effective number of *tactic clusters*: the kernel computes pairwise distance between personas' promoted tactic sets, clusters the population in tactic space, and applies the order-2 Hill number over cluster shares (as for `Ne_d`). The exact distance metric is **operator-tunable** (`population-policy/1.tactic_distance_metric`); the seed is **normalized edit/embedding distance over promoted EVOLVE-BLOCK text**. `Ne_t` falls toward 1 as promoted behaviour homogenises, regardless of headcount or niche spread.

**Combined, temporally smoothed metric.** `effective_population_size = harmonic_mean over the last eps_temporal_window of min(Ne_v, Ne_d, Ne_t)`. The harmonic mean over time is Wright's temporal-`Ne` construction — it is dominated by the *lowest* recent value, so a brief diversity bottleneck keeps the injection mandate armed until diversity genuinely recovers, not merely for one window. The result feeds `population-pressure-signal/1.effective_population_size` (`§4A` factor 6) and the `§4C` injection threshold; it is kernel-measured and read-only to personas.

## 4H. Continuous diversity maintenance (mitigates post-birth drift, OQ-POP-6)

`§4C` guarantees variety **at birth**. It does not, on its own, stop a cohort from drifting toward monoculture *after* birth. The convergence forces are: selection, imitation, and prestige-biased cultural transmission (all pull toward the locally-successful type); **habit-strength shielding** ([`08_KNOWLEDGE.md §14.2a`](08_KNOWLEDGE.md#142a-habit-strength-on-tactics-adr-0080), ADR-0080) — heavily-used tactics resist mutation, so once a population converges on a tactic set, habit *entrenches* the convergence by damping the very mutation pressure that would re-diversify it; and **cohort-migration correlation** ([`14_DECISIONS.md`](14_DECISIONS.md#adr-0074--tactic-lineage--promptops-per-tactic-version-dag-trial-records-per-tactic-rollback-cohort-migration-across-model-upgrades) ADR-0074) — personas sharing a body-family upgrade migrate tactics in correlated lockstep, a shared shock that pushes co-upgraded personas toward the same behaviour at once. v1.0 left post-birth drift as the honest residual behind OQ-POP-6 / OQ-PERSONA-1. This section adds a continuous loop, reusing the standard quality-diversity / evolutionary-computation toolkit for diversity preservation; it composes with the **cross-persona differentiation / similarity audit** ([`08_KNOWLEDGE.md §14.1a`](08_KNOWLEDGE.md#141a-identity-expression-safeguards-adr-0073), ADR-0073), which rolls back identity-driven evolution that homogenises personas — the per-persona control whose population-scale complement is the `Ne_t`-aware loop below.
...
9. **Concurrent duplicate birth is serialized.** Two wakes observe the same missing modelling capability before the first newborn has contributed or judged its situation. The first proposal is admitted; the second is REFUSED `persona_birth_integration_pending` without comparing either seed's role prose. After the first newborn publishes its verified model bytes, the gate clears. A later proposal still has to establish its own pressure, recruitment exhaustion, and niche distinctiveness.
...
|----|----------|-------|---------------|
| OQ-POP-1 | `pressure_score` aggregation weights across the seven factors (`§4A`) — operator-tunable or learned? | Population WG | v1.1 calibration |
...
| OQ-POP-7 | Adaptive mid-flight intuition — a mentor's applicability prior that updates *during* a task rather than only at skill-transfer mint — is the recorded remainder of R-PERSONA-5 (ADR-0080 deferred it "to the population-dynamics arc"). What is the safe shape for a population-scale, mid-task-adaptive intuition signal that does not bypass the receiver's DualProcessGate ([`08_KNOWLEDGE.md §13a`](08_KNOWLEDGE.md#13a-calibration-and-belief-revision-adr-0078))? | Evolution WG | v1.2 design |
...
| R-POP-8 | **Treasury→compute→population compound loop**: treasury earnings buy metered compute (x402), compute raises INV-7 budget headroom, headroom raises genesis pressure and (under `ceiling_from_budget`) the population ceiling, and a larger population earns more — a self-amplifying reproduction loop. Mirrored as R-ECON-COMPOUND in [`17_ECONOMY.md §13`](17_ECONOMY.md). | High | Medium | The §4F **treasury→compute→population invariant**: treasury-funded budget increases MUST NOT move `ceiling_from_budget` without a fresh operator cosignature per increase (no standing authorization); headroom may raise hosting capacity but never the ceiling absent the per-increase cosign; `ReplicationBound` rate/depth/population caps still bind (A-GEN26). | v2.0 |
| R-POP-9 | Concurrent persona wakes mint overlapping newborns before either birth has integrated, wasting model budget and destabilising the cohort. | High | Medium | Task-scoped integration serialization (`§4C`): admit one unresolved newborn at a time; clear only on verified contribution, signed judgment, explicit non-membership response, or lifecycle exit. | v1.1 |
...
- **A-GEN25** Corroborated gate fitness (`§4D`, ADR-0083): a candidate author whose judged-signal credit (`identity_expression`/`w_ide`) would carry it over `generativity_fitness_threshold` while its verified-outcome components alone fall below → REFUSED `author_not_generative`; judged signals capped at 25% of gate-consumed fitness and never flip eligibility absent verified-outcome support.
- **A-GEN26** Treasury→compute→population invariant (`§4F`, ADR-0083): a treasury-funded budget increase that would raise `ceiling_from_budget` WITHOUT a fresh per-increase operator cosignature → ceiling move REFUSED (hosting capacity MAY rise; ceiling does not); a standing/`operator_deferred` authorization does NOT satisfy the cosign.
- **A-GEN27** Active-population visibility: every funded persona turn receives the bounded, verified PersonaCards of the other ACTIVE members, preserving persona-authored descriptions and characteristics under context compaction. The kernel reports no inferred gap, role, rank, recruit, or birth action; a birth occurs only after the persona authors the required need/recruitment/genesis lineage.
- **A-GEN28** Task-scoped integration serialization: while one admitted newborn for the same environment/task has no later verified workspace/artifact contribution, signed situation judgment, explicit non-membership response, or lifecycle exit, a second proposal is REFUSED `persona_birth_integration_pending`; after one of those exact records is present, a later niche-distinct proposal may proceed through the ordinary gates.
...
class PopulationPressureSignal:
    ...
    improvement_blocking_gap_under_headroom: bool  # factor 7: a ContinuousRefinementMission objective no ACTIVE persona can advance AND BudgetState headroom exists (§4A, ADR-0071)
    # Aggregate
...
class PopulationPolicy:
    ...
    tactic_distance_metric: str = "evolve_block_edit_embedding"  # Ne_t pairwise distance (§4G, ADR-0083); seed = normalized edit/embedding distance over promoted EVOLVE-BLOCK text
...
class EffectivePopulationSizeEstimate:
    ...
    ne_tactic_diversity: float                     # inverse-Simpson effective number of tactic clusters Ne_t (ADR-0083)
    effective_population_size: float               # harmonic_mean over eps_temporal_window of min(Ne_v, Ne_d, Ne_t)
