...
**DESIGNED LIMIT — task families with no independent reference never promote.** Criterion (b) requires an independent reference: a shadow seed pathway or a post-hoc ground-truth signal. For task families where **neither exists** — OPEN_ENDED, RELATIONAL, and EXISTENTIAL work, where no STANDARDISED reference pathway produces a comparable verdict and no post-hoc ground truth ever resolves — an emergent acceptance pathway can **never satisfy (b) and therefore never promotes**. This is intentional, not an oversight: it is the orchestration-layer face of the ground-truth non-goal ([`00_VISION §10`](00_VISION.md#10-out-of-scope-by-design), "ground-truth without bench measurement" — the system honestly degrades trust rather than manufacturing certainty). In these families emergent pathways remain EMERGENT indefinitely, their verdicts carry the trust-calibrated discount indefinitely, and the §2b.1 curve still governs relative weight among them — the design degrades honestly where verification is thin rather than letting usage volume impersonate soundness evidence. Whether a *judged-work reference protocol* (e.g. a rotating panel run in shadow as the independent reference, with anti-Goodhart controls) could lift this limit without re-opening the judge-Goodhart channel is tracked as OQ-TASKS-7.
...
**Bounded contribution fairness, exact action availability, and response linkage.** A bounded coordination projection MUST reserve the newest verified persona-authored contribution from each represented ACTIVE participant before filling its remaining slots from the recent append-order tail. One persona's later burst of actions or receipts MUST NOT evict another active persona's still-relevant signed review merely because the context bound is full. This reservation MUST survive the separate model-prompt byte projection, MUST be admitted before larger optional mechanism catalogs or accumulated history, and MUST also expose the newest signed envelope for each other ACTIVE participant as a dedicated top-level peer-contribution fact so its authored content is not buried behind generic nested projections. Selection and byte allocation use only verified authorship, active membership, append order, and fixed bounds; they do not interpret, rank, or summarize the contribution. A request-bound action MUST appear in neither the signed situation action index nor the provider tool lease unless an exact pending request exists for the acting persona, environment, and task; the same mechanical predicate governs both surfaces, and the exact request ID plus observation hash MUST remain model-visible ahead of that action under prompt-byte pressure. An unavailable request MUST NOT be replaced with an implicit or invented request identifier. A verified pre-membership invitation wake likewise binds exactly one signed `invitation_id`: the provider-facing membership-decision schema exposes the persona-authored `commit_membership` boolean but omits that transport-owned identifier, and dispatch injects the verified nested invitation identifier rather than asking the persona to distinguish it from the carrier lineage `event_id`; any conflicting extra identifier fails closed. Native and structured action carriers enforce the same binding. A same-run resource-recovery wake MUST also carry forward the exact verified task-entry materialized-outcome baseline from persisted run authority; losing that baseline MUST fail closed rather than turn already verified workspace bytes into a new forced-production obligation. When a persona explicitly addresses a response to another active participant and names its exact current signed wake in `in_reply_to`, that wake identifier is a valid causal response reference even though it is not a `PersonaCommunication` parent identifier. The transport retains the reference in the signed response, leaves the communication-parent hash empty, publishes the response, and routes the ordinary recipient wake. Any different unresolved identifier fails closed. A resource grant, readiness gate, or host heuristic still does not select a reviewer or mint a review turn.
