...
`PhysicalAsset` lives alongside `ArtifactBundle`, not inside it. The bundle holds the digital design; the asset holds the physical reality. The asset's `state_kind` FSM ranges over **physical** realisation states only (its `related_bundles` carry the designs/plans/recipes); producing or advancing those design bundles is a digital bundle-lifecycle transition (`07_ARTIFACTS §6`) and **does not** advance `current_state` — so it never engages the physical-attestation floor (`01_KERNEL §2.5.1`), regardless of `physical_harm_class`. The credentialed-attestation floor attaches only to `current_state` advancement; the interval where designs are accepted but the asset is not yet built is the `digital_complete_physical_in_progress` project state (`§4.1`). Completion ceremony (§4.1) is extended: a project that produces a `PhysicalAsset` cannot reach `completed` until either the asset reaches a terminal state in its `state_kind` FSM, OR the project explicitly declares the asset out of scope (signed `physical_asset_out_of_scope` event). This closes the prior gap where "design done" silently meant "project done."
