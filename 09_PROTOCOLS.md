...
**Reducing the single-host bottleneck (normative; ADR-0062).** The single-host rule is a deliberate centralisation point and this design's weakest decentralisation property. PersonaOS ships an optional **replicated-host** posture as a STANDARDISED seed coordination shape: the host designates a `standby_replica_set` of peer nodes holding a warm, signed Blackboard shadow, and **BFT quorum sign-off** promotes a standby on host loss without full replay — adopting the consensus pattern proven by OpenCLAW-P2P (`§3G.6`) and composing with `MultiPrincipalAttestationQuorum` ([`05_ENVIRONMENT §12c.4a`](05_ENVIRONMENT.md#12c4a-multiprincipalattestationquorum--multi-principal-ratification)). This removes the *availability* single point of failure. **What it does not abolish (navigated, not deferred — V.8):** full leaderless multi-writer BFT against genuinely adversarial independent nodes is a hard distributed-systems + trust reality, not a feature shippable as a guarantee. Sequence assignment still flows through one promoted host at a time (split-brain prevented by majority quorum, R-PROTOCOLS-14), and an independent node's *honesty* is **trust-calibrated** (J5/C3), never assumed — its signatures verify identity/integrity; its truthfulness enters at calibrated trust and degrades on disconfirmation. Captured as ADR-0062; recorded as a V.8 navigated-reality PASS in [`13_DESIGN_VALIDATION.md`](13_DESIGN_VALIDATION.md).
...
This pillar records how PersonaOS's storage / distribution / discovery substrate maps onto the agentic-world standards maturing in 2025–2026. Most of it is **informative** (non-normative): it validates existing PersonaOS choices and names the standards an operator MAY adopt at the federation boundary. **Two elements are now normative substrate**, lifted out of "informative" by the one-global-object-space realignment: (1) the **W3C DID global handle** over each entity's kernel-minted id — the node-agnostic resolution + verification key required by J1 (`01_KERNEL §4.4`, ADR-0067); and (2) **attenuated UCAN capability tokens** chained from a persona's DID as the substrate mechanism for **cross-env / cross-node privilege delegation** — "grant persona/principal P the capability to discover / read / co-edit / submit-work-to entity X" (ADR-0068; used by `CrossEnvTaskDelegation`, `03_TASKS §4.5`, and the node intake gate, `01_KERNEL §13`). A UCAN capability is verified against the granting entity's `AccessPolicy` (`§3G.3`) and composes most-restrictive-wins; it can attenuate but never widen a grant. Everything else below remains an interoperability target, not a substrate requirement.
...
**Open source text cannot suppress discovery.** A task or environment MAY retain exact principal-authored text whose length exceeds the compact record fields. The public record builder MUST project that text to a deterministic UTF-8-safe human label/description within the wire bounds while the full authoritative object remains available through its signed/id-addressed source. It MUST perform this projection before record construction: an otherwise valid long task, environment name, persona description, or telemetry label MUST NOT abort the publication transaction or delay the node, persona, environment, task, and artifact records behind it.
...
#### 3G.2a Live announcement-locator profile

An HTTP announcement locator is an **optional, replaceable last-resort first-contact cache**, never a record authority, identity root, or continuously required network hub. Direct configured peers, local discovery, and the shared libp2p/Kademlia plane are primary. A public node signs an exact `personaos-node-announcement/1` with its self-certifying kernel-master key only when those primary routes fail to yield an independently verified PersonaOS peer after their bounded startup opportunity, or later become unavailable. Merely joining a routing table or connecting to generic bootstrap peers is **transport readiness**, not successful PersonaOS discovery, and MUST NOT suppress last-resort first contact while zero remote PersonaOS identity or provider route has verified. While fallback is active, the reference profile refreshes every **15 seconds** under a **45-second signed lease**. Once a direct route succeeds or a peer-bound remote PersonaCard/provider route is admitted, reads and announcements to the locator stop; a previously announced fallback lease expires naturally (or MAY be withdrawn with a higher-sequence, signed `public_discovery=false` record). Orderly shutdown sends the same signed withdrawal when a live fallback lease exists. Crash, partition, or failed withdrawal falls back to bounded lease expiry—no node may remain active indefinitely because a locator retained an old row.

A locator MUST prune expired leases before every read, serve `Cache-Control: no-store`, expose a monotonic directory `revision` and a bounded `refresh_after_s` hint (three seconds in the reference profile), support bounded cursor pages, and support `order=recent&status=active` so a just-announced node enters the first page instead of waiting behind a stable lexical window. A complete successful page walk atomically replaces that locator's prior snapshot. A partial bounded walk merges newly verified leases with still-live prior leases; a transient locator failure likewise retains only still-live signed leases and MUST NOT invent either additions or deletions. Counts describe the current snapshot, never a historical maximum.

A live consumer MUST first give direct/local/IPFS and libp2p/Kademlia discovery a bounded cold-start opportunity. It queries an HTTP locator only when that opportunity yields neither a verified P2P data route nor a healthy direct node read. The reference browser gives transports a **4.5-second minimum warm-up**, but MUST NOT race a configured first DHT rendezvous pass that is still in flight: it waits for that pass to complete, with a **20-second absolute first-contact ceiling** so a wedged peer stack cannot suppress fallback forever. A locator-introduced node that supplies a complete independently verified provider inventory becomes a direct peer route; locator polling then stops. The consumer rechecks route health locally and re-enters fallback only after all such routes are unavailable. While fallback is active it follows the locator's bounded refresh hint subject to a local anti-amplification floor (10 seconds in the reference browser), verifies announcement signatures in parallel, and renders verified node presence before fetching the heavier provider inventory. A material node-set change (join, withdrawal, route/reachability change, record-count change, or lease expiry) triggers persona/environment record discovery immediately. A lease-only heartbeat updates freshness without forcing that heavy pass. Provider inventories and their persona/environment records have their own signed expiry and are retired together, including their now-expired direct-route source tags; exact JSON remains an audit surface, while the human display leads with verified names, workspaces, and state. Every locator result remains only a route hint: provider inventories, records, access policies, identities, and artifact bytes are independently verified at the reached node.
...
*A `ReachabilityProfile` (`reachability-profile/1`) advertises a node's transports (multiaddrs) and its observed reachability class: `public` (directly dialable — a public IP or forwarded port), `nat_private` (behind NAT / CGNAT / a home router — the common case), or `intranet_only` (link-local; never leaves the LAN, by policy or topology). A public provider MAY sign multiple independently dialable WSS and/or circuit routes for the same stable PeerId; consumers try the bounded address set and do not treat one failed endpoint as failure of the peer identity. For `nat_private` nodes the profile specifies normative use of **libp2p circuit-relay v2** (a willing relay peer forwards the initial connection) plus **DCUtR hole-punching** (the two ends upgrade to a direct connection once introduced), bootstrapped through a configured set of **bootstrap / rendezvous** peers. The resolver (`§3G.2`) returns these relay-reachable multiaddrs — not just a host id — so a record published from a NAT-private home node is dialable from elsewhere on the internet. The profile is signed and access-gated like any other record.*
...
| R-PROTOCOLS-16 | An untrusted announcement locator serves stale nodes or delays a new node behind a large directory window. | Medium | High | `§3G.2a`: self-signed short leases, signed graceful withdrawal, recent-first bounded pages, no-store responses, atomic current snapshots, failure-safe lease retention, browser-side signature/expiry verification, and change-triggered record hydration. | v1.1 reference profile. |
