...
tool. Each local record states `execution_mode` (`library_only` or
`callable_tool`), whether that exact artifact is still the latest signed
registration (`currently_registered`), and a matching action on the current
bridge (`callable_action`, otherwise null). A library's recorded `tool_name`
is its acquisition label, not a callable action. Superseded and unregistered
acquisitions remain history. Malformed or mismatched descriptors give unknown
mode and registration; an incomplete lineage gives unknown registration, never
a false claim that a tool is available. Action matching includes the exact
artifact, owner, environment and descriptor hash, not just its name. This read
does not install, wrap, select or invoke anything, and dispatch still checks
authority and execution evidence when a persona chooses to call a tool.
...
Repeated reads use an earlier exact result already present in the same request.
An unchanged result may reference it directly. A changed result may carry an
exact delta: the earlier result index, removed fields, replacement fields and
appended list items. Applying those mechanical operations recovers the complete
current body and its read-version hash. Both reads must name the same resource,
query and authority; no peer or expired scope supplies a base. Changes inside a
list, late arrivals and shorter lists are complete field replacements, never
...
compacting away a base cannot leave a reference claiming it is still in context.
Malformed, changed, wrong-scope and forward references cannot be reconstructed
as the stated current read. Persona-authored checkpoints
...
See `registry/SCHEMAS.md` (generated by `tools/registry.py render --write`; 1328 rows; registry sha256 a1d792483eee137e6b162e9b5c8b06c79db3c33d0bb2d192aa497d4af5501fb7).
