...
An HTTP announcement locator is an **optional, replaceable first-contact cache**, never a record authority or identity root. A public node signs an exact `personaos-node-announcement/1` with its self-certifying kernel-master key and announces once at startup. The reference live profile refreshes every **15 seconds** under a **45-second signed lease**. Orderly shutdown sends a higher-sequence, signed `public_discovery=false` withdrawal; a locator removes that kernel immediately. Crash, partition, or failed withdrawal falls back to bounded lease expiry—no node may remain active indefinitely because a locator retained an old row.
...
A live consumer queries at initial paint and at the bounded refresh hint while warming or empty, verifies announcement signatures in parallel, and renders verified node presence before fetching the heavier provider inventory. A material node-set change (join, withdrawal, route/reachability change, record-count change, or lease expiry) triggers persona/environment record discovery immediately. A lease-only heartbeat updates freshness without forcing that heavy pass. Provider inventories and their persona/environment records have their own signed expiry and are retired together; exact JSON remains an audit surface, while the human display leads with verified names, workspaces, and state. Every locator result remains only a route hint: provider inventories, records, access policies, identities, and artifact bytes are independently verified at the reached node.
