...
Update `../../visual-manifest.json` and the matching text reading in `../../VISUAL-GUIDE.md` after a semantic change. Run `python3 scripts/validate_visuals.py` from the repository root. With CairoSVG installed, add `--render-dir /tmp/ai-personas-visuals` and inspect every output at full size. The validator catches structural regressions; it does not prove semantic equivalence or detect every visual collision.

Do not change proposal requirements merely to simplify a diagram. Preserve refusal, negotiation, revalidation, and honest-stop paths. Keep source-derived requirements separate from proposed extensions. The original raster artwork and Mermaid diagrams remain in the original proposal for provenance.
