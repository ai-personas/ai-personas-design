...
**`outward_tier` tightens, never lowers the safety floor.** Visibility tier governs discovery and access only. Raising `outward_tier` (e.g. to `federation` / `public`) may **add** requirements but MUST NOT lower any physical-harm floor (`01_KERNEL §2.5`); a local, undistributed `project_only` bundle and a `public` one carry the *same* `physical_harm_class` floor on the asset they prescribe (most-restrictive-wins, `01_KERNEL §2.1`). Undistributed work stays off all discovery planes by default (`project_only`; `09_PROTOCOLS §3G.4`) — visible to its owning env and granted principals, never to peers globally.

**Attestation grants access (additive).** When a design bundle is shared at `outward_tier ∈ {federation, public}` and it prescribes a `PhysicalAsset` whose distributed advancement requires a credentialed `ExternalAttestation` (`04_PROJECT §26a.3`), the kernel grants the named attestor (`ExternalAgent`, `04_PROJECT §26a.1`) at least `read` on the bundle for the duration of the attestation, via a signed `AccessGrant` — so whoever must attest can access exactly what they attest to. The grant is purely additive (it widens no one else's access) and composes by most-restrictive-wins with the bundle's other grants.
