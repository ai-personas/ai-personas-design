...
**Version 1.1 — Rust-only · 16 September 2026**  
**Canonical design:** this complete Rust-only specification replaces the previous implementation specification. The prior Python-target recommendation, inherited-capability assumptions, reference-code test claims, and source mapping do not apply. The agreed persona/group behavior is retained.  
...
A `Work` represents a need in an `Environment`; a `Run` is an execution episode/funding binding for that work. Continuing or changing models does not erase the work's mandate. A new task creates a new work-scoped active context unless continuity is explicitly selected.
...
### 6.5 No-progress handling

The runtime can observe no new execution receipts, artifact revisions, resolved requests, or recorded decisions over a configured interval. It reports those mechanical facts, not “your ideas are useless.” A persona or evaluator judges usefulness. Bounded self-wakes may support continuing exploration; self-wakes cannot escape root budgets or run at an unbounded idle cadence.
...
| Grant/Budget | Rust-scoped authority, root allowances, revocation, reservations and settlement to implement in the existing store |
...
|---|---|
| Work activity | `ready`, `active`, `waiting`, `paused`, `quiescent`, `canceled`; does not encode acceptance |
...
| Action | `recorded` → `admitted` / `denied`; admitted → `queued` → `running`; then `succeeded`, `failed`, `canceled`, or `effect_unknown` |
| Birth | `proposed` → `admitted` / `refused`; admitted → identity persisted + initialization pending; initialized identity still needs separate membership consent |
...
### 17.3 Actor loop with fences — Rust-oriented pseudocode

The following is an architectural sketch, **not compiled Rust and not a patch**. Helper names describe contracts to implement around the existing node/store; they are not asserted existing APIs.

```rust
async fn handle_wake(node: &Node, persona: PersonaId, wake_id: EventId) -> Result<()> {
    let lease = node.acquire_fenced_lease(persona).await?;
    let result = async {
        let wake = node.load_authorized_wake(wake_id)?;
        let state = node.read_consistent_state(persona, &wake)?;
        let context = node.compile_persona_context(&state)?;
        let call = node.reserve_model_call(&state, lease.fence())?;

        // No SQLite transaction or blocking store mutex lives across this await.
        // Lease heartbeat/cancellation is supervised separately.
        let response = node.provider_decide(&context, &call).await;
        node.record_response_and_usage(&call, &response)?;
        let response = response?;

        for (ordinal, proposal) in response.decision.actions.into_iter().enumerate() {
            // Stable IDs are durably bound to the call + action ordinal.
            // Model output cannot mint actors, grants, roots, or resource pools.
            let op = node.bind_model_operation(&call, ordinal, proposal)?;
            node.admit_in_store_transaction(&op, lease.fence())?;
            // First failure stops the remaining batch, preserving earlier receipts.
        }
        node.record_wake_handled(&wake)?;
        Ok(())
    }.await;
    node.release_lease_if_owner(&lease).await?;
    result
}
```

A production implementation must preserve the original error if lease cleanup also fails, retain both diagnostics, renew leases while awaiting providers, and recover after task cancellation/panic. Receipt persistence failures retain uncertain accounting instead of refunding usage. `record_wake_handled` records dispatch handling only: it does **not** replace the persona-authored `input.acknowledge` cursor or acknowledge unread input.

The current Rust protocol says a failed action stops the remainder of its decision. Preserve that behavior unless explicitly versioned; already admitted actions remain durable. Add an explicit quiescent/yield disposition so an empty action array does not automatically create unbounded model-call churn. No hidden task planner should select the semantic recovery. [R05]

The decoder never trusts actor IDs, grants, or root budgets supplied by the model. A batch is admitted in order, with bounded internal atomic mutations only where specified and no external effects inside a transaction. Dependent calls reference actual returned IDs, never guessed future records.
...
| Birth/membership | Existing birth proposal/admission; invite/accept/leave | Replication scope, lineage, capacity, initialization funding, separate consent |
...
| Plumbing | Supply/hot-water/drain/vent intent at agreed detail, fixture/network schedules and sizing basis, connected/coordinated routes and access |
| HVAC | Room/system assumptions, loads, selection/distribution/ventilation/control design at agreed detail, appropriate reproducible performance evidence |
| Electrical | Lighting/outlets/equipment supplies, circuits/panel schedules, load/protection/grounding basis at agreed detail, cross-document consistency |
...
| `src/delivery.rs` | Extend the existing delivery path with addressed birth/invitation/review/commitment and authoritative-change notifications. Preserve durable input identity, paging and explicit acknowledgements; deduplicate wakeups without discarding unhandled inputs. |
...
|---|---|---|
| P0 — baseline/contract | Inventory existing semantics, freeze versions, adopt this spec and compatibility map | One target implementation, preserved historical evidence, no duplicated authority systems |
| P1 — persistence/authority | Transactional admission, fences, outbox, reservations, cancellation, scoped API | M01–M07, M11–M13 on real runtime paths |
| P2 — persona/group carriage | Individual agendas, relationship fragments, exact agreement endorsements, commitments and per-work contexts | M08 plus a small repeated B01/B02/B04 diagnostic; no claim of full competence |
| P3 — real cooperation/evidence | Artifact/criteria/scope binding, change invalidation, feedback/handoff, truthful projections | M02/M09/M10 and genuine B05/B07 |
| P4 — learning/population | Retained correction, birth-work/contribution links, recruitment/consent/lifecycle | B08–B10 within conserved resources |
| P5 — complete product | Providers/tools/UI integration and complete house + unrelated tasks | M14–M16, B06/B11/B12, fresh repeats |
| P6 — release | Coherent runtime/schema/UI/docs/package, restore/security/load checks and cost report | Declared behavioral thresholds met; no fabricated currentness or external authority |

Start with a small Rust public-API cooperation/correction fixture, preserving the branch’s original failures and criteria. Diagnose missing context/contract or model limitations. Evidence from another runtime is not a regression result for this branch. Do not spend first on a larger house run, another language, or extra personas. UI development proceeds against the same projections as each milestone, not a separate completion model.

## 25. Delivered materials and implementation status

`AI-PERSONAS-RUST-FINAL-SPEC.md` is the only normative design in this package. `AI-PERSONAS-RUST-FINAL-SPEC.html` renders the same content. `WORKSPACE.html` is the unchanged, backend-independent fixture UI retained from the prior proposal: it illustrates different teams, agendas, agreements, commitments, births/restraint and evidence staleness, not a Rust backend integration.

`RUST-BASELINE-AND-CHANGES.md` summarizes the scope correction and verified baseline. `verification.json` reports checks performed for this revised document package. No earlier reference-code test result or UI fixture count is represented as evidence for the Rust runtime. The earlier Python reference implementation is deliberately omitted.

No repository files were modified. No Rust build or test was run: `cargo` and `rustc` were unavailable on the execution PATH. There were no live provider calls, engineering tools, house simulations, persona births, or learning evaluations. The illustrative Rust loop is not compilable drop-in code. The release gates in Section 24 remain to be implemented and executed on the pinned Rust branch.
